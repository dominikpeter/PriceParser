...
def modify_dataframe(df, idHersteller_Columns = ['FarbId',
                                                'AusführungsId',
                                                'Art_Nr_Hersteller',
...
def prepare_data(join_df_path,
    ...
    join_df = modify_dataframe(join_df, idHersteller_Columns)
    main_df = modify_dataframe(main_df, idHersteller_Columns)
...
def join_meta_data(main_df,
    ...
    if sales:
        ...
        sales_query = pp.load_sql_text(os.path.join(path,"SQL", sales_sql))
        sales = pp.sql_to_pandas(con, sales_query)
        main_df = main_df.merge(
            sales, how='left', on=on,  suffixes=('', '_y'))

    if meta:
        print('Getting Meta Data from Database...')
        meta_query = pp.load_sql_text(os.path.join(path, "SQL", meta_sql))
        meta = pp.sql_to_pandas(con, meta_query, parse_dates=['Erstellt_Am'])
        main_df = main_df.merge(
            meta, how='left', on=on,  suffixes=('', '_y'))
...
def main(settings, currentpath):
    ...
    sales_sql         = settings['SQL']['Sales']
    meta_sql          = settings['SQL']['Meta']
    sql_on            = settings['SQL']['Join']
    ...
    if re.match(".+\.sql", main_file_pattern):
        try:
            print("Getting Main File from Database..")
            query_ = pp.load_sql_text(os.path.join(currentpath,"SQL", main_file_pattern))
            main_df = pp.sql_to_pandas(
                            pp.create_connection_string_turbo(
                                    'CRHBUSADWH02', 'AnalystCM'),
                                        query_)
            #other logic than XML Files
    ...
    for i in files_to_match:
        ...
        main_df = prepare_data(
                        ...
                        chunksize=chunksize,
                        idHersteller_Columns = idHersteller_Columns)
